...
class BaseTransformer(nn.Module):
    ...
    def forward(self, sources, targets, source_mask=None, tgt_mask=None):
        """
        ...
        """
        sources, targets = sources.transpose(0, 1), targets.transpose(0, 1)
        tgt_mask = tgt_mask.squeeze()
        y = self.decoder(
            targets, self.encoder(sources, source_mask), source_mask, tgt_mask
        )
